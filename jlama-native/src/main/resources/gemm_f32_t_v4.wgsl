struct Params {
    ...
    ldb: u32,      // Leading dimension of B (n for k x n, since transposed)
...
@group(0) @binding(1) var<storage, read> B: array<f32>;  // B: k x n row-major (transposed input, B^T)
...
fn main(
    ...
    for (var kt = 0u; kt < params.k; kt += BK) {
        ...
        if (tile_jj + local_id.x < params.n && kt + local_id.y < params.k) {
            Bs[local_id.x * BK + local_id.y] = B[(kt + local_id.y) * params.ldb + (tile_jj + local_id.x)];
        } else {
