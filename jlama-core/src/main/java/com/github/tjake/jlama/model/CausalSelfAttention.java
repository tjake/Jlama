    ...
    public AbstractTensor forward(
        ...
        try (AbstractTensor flashAttn_m = m.makeTensor(c.numberOfHeads);
            ...
            queryAttnBias.ifPresent(bias -> TensorOperationsProvider.get()
                    .accumulate(query, bias, c.embeddingSegmentStart(), c.embeddingSegmentLength()));
            keyAttnBias.ifPresent(bias ->
                    TensorOperationsProvider.get().accumulate(tmpKey, bias, c.kvSegmentStart(), c.kvSegmentLength()));
            valueAttnBias.ifPresent(bias ->
                    TensorOperationsProvider.get().accumulate(tmpVal, bias, c.kvSegmentStart(), c.kvSegmentLength()));

            key.copyFrom(
                    tmpKey,
                    tmpKey.getOffset(c.kvSegmentStart()),
                    key.getOffset(c.kvSegmentStart()),
                    c.kvSegmentLength());
            val.copyFrom(
                    tmpVal,
                    tmpVal.getOffset(c.kvSegmentStart()),
                    val.getOffset(c.kvSegmentStart()),
                    c.kvSegmentLength());
            ...
            for (int i = c.headStart(); i < c.headEnd(); i++) {
                value.copyFrom(
                        v0,
                        v0.getOffset(c.maybeMapToGroupHead(i) * c.headSize),
                        value.getOffset(i * c.headSize),
                        c.headSize);
                float a = TensorOperationsProvider.get()
                                .dotProduct(
                                        query, k0, i * c.headSize, c.maybeMapToGroupHead(i) * c.headSize, c.headSize)
                        * attentionScale;
                flashAttn_m.set(a, i);
                flashAttn_l.set(1, i);
            }
            ...
            for (int i = 0; i < position; i++) {
                ...
                for (int h = c.headStart(); h < c.headEnd(); h++) {
                    ...
                    if (a > flashAttn_m.get(h)) {
                        ...
                        TensorOperationsProvider.get()
                                .sxpby(e, vv, value, c.maybeMapToGroupHead(h) * c.headSize, h * c.headSize, c.headSize);
                        flashAttn_l.set(1 + e * flashAttn_l.get(h), h);
                        flashAttn_m.set(a, h);
                    } else {
                        float e = (float) Math.exp(a - flashAttn_m.get(h));
                        TensorOperationsProvider.get()
                                .saxpy(e, vv, value, c.maybeMapToGroupHead(h) * c.headSize, h * c.headSize, c.headSize);
